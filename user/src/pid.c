 ...
 * @retval pid 错误状态
 */
 PID_Status_e PID_Param_Init(PID_Info_TypeDef *Pid,float para[PID_PARAMETER_NUM])
{
...
//  }
  ...
  /* update the error */
	...
	Pid->Err[0] = Pid->target - Pid->measure;
		if(Pid->Err[0]>=1000000) Pid->Err[0]=Pid->Err[0]-589824*2;
	else if(Pid->Err[0]<=-1000000) Pid->Err[0]=Pid->Err[0]+589824*2;
  if(fabsf(Pid->Err[0]) >= Pid->param.Deadband)
  {
      ...
      VAL_LIMIT(Pid->Integral,-Pid->param.limitIntegral,Pid->param.limitIntegral);//积分限幅
      ...
      VAL_LIMIT(Pid->Output,-Pid->param.limitOutput,Pid->param.limitOutput);//最大输出限制
		}
		else if(Pid->type == PID_VELOCITY)//增量式PID
		{
      /* Update the Proportional Output,Integral Output,Derivative Output */
